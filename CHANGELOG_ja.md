...
### 追加
- メタデータ取得時に削除済み動画（削除済みまたはアカウント停止）を検出し、動画カードに「削除済み」バッジを表示
- 削除済み動画検出時に動画タイトル・チャンネル名付きの通知を表示
- 削除済み動画を一括DLから自動スキップ（非公開動画と同様）

### 修正
- 配信終了後の起動時に、前回セッションの古いinfo.jsonを基にライブ配信と誤検知される問題を修正
- ライブ配信判定の前に古いタイムスタンプ付きinfo.jsonをクリーンアップするよう変更し、常に最新のメタデータで判定するよう修正

## [1.0.12] - 2026-02-15

### 追加
- メタデータ取得時に配信予定のライブ配信を検出し、動画カードに「配信予定」バッジを表示
- 起動時に配信予定の動画も配信中と同様に再チェックするよう対応
- 開発環境専用のリセットボタンをヘッダーに追加

### 修正
- 保存先フォルダ未設定時に動画追加ボタンが有効のままになり、メタデータ取得が失敗する問題を修正
- 配信予定検出フラグのレースコンディションを修正（stderr文字列によるフォールバック検出を追加）
- 一括DL時に配信中・配信予定の動画をダウンロードしようとする問題を修正（自動スキップ）
- 個別DL時に配信予定の動画をブロックしていなかった問題を修正

### 変更
- 開発用とリリース用で設定ディレクトリを分離（デバッグモードで `-dev` サフィックス付加）
